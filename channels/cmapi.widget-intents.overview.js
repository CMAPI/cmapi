cmapi.overview["cmapi.widget-intents.overview"] = {
	...
	"sections": [{
		...
		"paragraphs": [
			"In keeping with best practices regarding OWF message payloads, Intents payloads should always be an Object, with a property called data that contains the literal data of the type described by the dataType property of the intent. For example:",
			"<pre><code class=\"javascript\">OWF.Intents.startActivity({<br/> action: 'view',<br/> dataType: 'application/vnd.google-earth.kml+xml' <br/>},<br/> {<br/> data: '<?xml version=\"1.0 \"?><kml>…</kml>' <br/>},<br/>function(dest) {<br/> … <br/>});</pre></code>",
			"This approach also allows for the distribution of always optional metadata. For the Common Map Widget API, the property containing this metadata is common_map. This property is used to group metadata that is specifically related to the Common Map Widget API, reduce possible conflicts, and avoid polluting the top-level payload object (which is not specific to the Common Map Widget API). For example:",
			"<pre><code class=\"javascript\">OWF.Intents.startActivity({<br/> action: 'view',<br/> dataType: 'application/vnd.google-earth.kml+xml' <br/>},<br/> {<br/> data: '<?xml version=\"1.0 \"?><kml>…</kml>',<br/> common_map: {<br/> overlayId: 'abc123',<br/> featureId: 'def456',<br/> name: 'My KML', <br/> zoom: true <br/>},<br/> function(dest) {<br/> … <br/>});</pre></code>",
